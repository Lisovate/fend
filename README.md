...
- **Linux spike:** Linux host work lives separately in Rust under `linux/`; the crate now exposes both `fend-linux` and a Linux `fend` binary, can render doctor output, inspect launch plans, run launch-time preflight before supervising QEMU/`virtiofsd`, execute one-shot disposable `fend <command>` runs, and verify host-to-`fendd` command execution with bounded smoke sessions; launch uses disposable rootfs writes, `scripts/prepare-linux-x86_64-runtime.sh` builds the x86_64 runtime artifacts, `scripts/linux-qemu-spike.sh` captures the manual QEMU/KVM launch shape, and `scripts/test-linux-spike-scripts.sh` covers host-independent checks.
