...
pub fn stop_usage_for(program: &str) -> String {
    format!(
        "usage: {program} stop [options]\n\noptions:\n  --project path         Stop the daemon-managed VM for this project. Default: current dir.\n  --all                  Stop every daemon-managed VM.\n  --run-dir path         Legacy: stop a specific direct-boot run dir.\n                         Default for run-dir mode: $FEND_RUN_DIR or /tmp/fend-linux.\n  --timeout sec          Grace period before SIGKILL. Default: 3.\n\nWhen --run-dir is given, the legacy direct-boot stop path is used.\nOtherwise the daemon is queried; if no daemon is running, this is a no-op.\n"
    )
}

pub fn status_usage() -> String {
    status_usage_for("fend-linux")
}

pub fn status_usage_for(program: &str) -> String {
    format!(
        "usage: {program} status\n\nList warm VMs currently managed by the host daemon, with their project, vsock CID,\nstate, active-session count, and idle time. If no daemon is running, prints an empty list.\n"
    )
}
...
fn parse_stop(mut args: VecDeque<String>) -> Result<CliCommand, CliError> {
    ...
    while let Some(arg) = args.pop_front() {
        match arg.as_str() {
            ...
            "--run-dir" => options.run_dir = Some(value_path(&mut args, "--run-dir")?),
            "--timeout" => options.timeout_secs = Some(value_number(&mut args, "--timeout")?),
            "--project" => options.project = Some(value_path(&mut args, "--project")?),
